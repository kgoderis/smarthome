 ...
 *
 */
public class DateTriggerHandler extends BaseModuleHandler<Trigger>implements TriggerHandler {

    private final Logger logger = LoggerFactory.getLogger(DateTriggerHandler.class);

    private RuleEngineCallback callback;
    private CallbackJob job;
    private DateExpression dateExpression;

    public static final String MODULE_TYPE_ID = "DateTrigger";
    private static final String CFG_DATE_EXPRESSION = "dateExpression";

    public DateTriggerHandler(Trigger module) {
        super(module);
        String dateExpression = (String) module.getConfiguration().get(CFG_DATE_EXPRESSION);
        try {
            this.dateExpression = new DateExpression(dateExpression);
